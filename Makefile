...
$(GOPATH1)/bin/node_exporter:
	mkdir -p $(GOPATH1)/bin && \
	cd $(GOPATH1)/bin && \
	tar -xzvf $(SRCPATH)/installer/external/node_exporter-stable-$(OS_TYPE)-$(shell uname -m | tr '[:upper:]' '[:lower:]').tar.gz && \
	cd -
...
gen/%/genesis.json: gen/%.json gen/generate.go build
...
gen/mainnet/genesis.json: gen/pregen/mainnet/genesis.csv build
	mkdir -p gen/mainnet
